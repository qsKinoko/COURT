...
class ARBITER:
    def __init__(self,strategy=None):
        instr = "You are a professional arbiter for Wiki user edit log detection. I have two detection systems: one based on anomaly detection and the other based on misuse detection. These systems have produced conflicting results. I will provide you with the known patterns of vandalic editing behavior, the original test log, the anomaly detection system's result and explanation, and the misuse detection system's result and explanation. Using this information, your task is to confirm the final detection result. Note that while insights from the anomaly detection system are still valuable, the misuse detection system's input should carry greater weight in your decision-making process.\n"
        instr += "You need to determine which of the following categories the test log belongs to:\n1) High Threat: Logs that align with one or more known patterns of vandalic editing behavior and have sufficient evidence of vandalic edits; 2) Low Threat: Logs that do not fully align with any known patterns of vandalic editing behavior but potentially containing vandalic edits; 3)Benign Anomaly: Logs that show anomalies but no signs of vandalic editing; 4) Benign Activity: Logs with no anomalies or signs of vandalic behavior; 5) Unknown Threat: Logs that might contain vandalic edits but do not align with any known patterns of vandalic editing behavior. This category should be considered when the misuse detection system classifies the log as benign, while the anomaly detection system classifies it as a threat.\n\n"
        instr += f"The known patterns of vandalic editing behavior:\n{strategy}\n\n"
    ...
    def arbit_experiments(self):
        ...
        test_result_file = open(glob.glob(os.path.join(args.output_dir,"test*"))[0],'r')
        ...
        arbitration_result_record = open(os.path.join(args.output_dir,f"arbitration_{current_time}.jsonl"),'w')
            ...
            if gan_test_result["Judgement"] != test_result["Judgement"]:
                ...
                while response_text=="":
                    ...
                    if json_match:
                        ...
                        print(arbit_result['Judgement'],ground_truth['label'])
